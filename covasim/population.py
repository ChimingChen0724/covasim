...
def make_randpop(sim, use_age_data=True, use_household_data=True, sex_ratio=0.5, microstructure=False):
    ...
    # edited!!
    # old: directally called different contact functions
    # new: apply factory pattern, add 3 if statement to separate different situations
    # just call "contract" objects and pass the "type" value to call the correct methods

    # create new object that use the MakeContracts class
    Contracts = MakeContracts()

    if microstructure == 'random':
        contacts, layer_keys = Contracts.make_contacts(
            'random_contact', pop_size, sim['contacts'])
    elif microstructure == 'clustered':
        contacts, layer_keys, _ = Contracts.make_contacts(
            'microstructured_contact', pop_size, sim['contacts'])
    elif microstructure == 'hybrid':
        contacts, layer_keys, _ = Contracts.make_contacts(
            'hybrid_contact', pop_size, ages, sim['contacts'])
    else:  # pragma: no cover
...
# reason: some files in this project was already applied creational patterns like "builder" and "factory"
# Also, three make_contacts function can be united together
...
class MakeContracts:
    ...
    # make function here to decide which function to call based on the input
    def make_contacts(self, type, pop_size, age, contacts):
        if type == "random_contact":
            contact_list, layer_keys = Contacts.make_random_contacts(
                pop_size, contacts)
            self.contact_list.append(contact_list)
            self.layer_keys.append(layer_keys)
            return self.contact_list, self.layer_keys
        elif type == "microstructured_contact":
            contact_list, layer_keys, _ = Contacts.make_microstructured_contacts(
                pop_size, contacts)
            self.contact_list.append(contact_list)
            self.layer_keys.append(layer_keys)
            self.clusters = _
            return self.contact_list, self.layer_keys, self.cluster
        elif type == "hybrid_contact":
            contact_list, layer_keys, _ = Contacts.make_hybrid_contacts(
                pop_size, age, contacts)
            self.contact_list.append(contact_list)
            self.layer_keys.append(layer_keys)
            self.clusters = _
            return self.contact_list, self.layer_keys, self.cluster

# add new Contacts class here to unite 3 different contacts methods
# easy to call
...
class Contacts:
    ...
    def make_hybrid_contacts(pop_size, ages, contacts, school_ages=None, work_ages=None):
        ...
        h_contacts, _, clusters = Contacts.make_microstructured_contacts(
        ...
        s_contacts, _ = Contacts.make_random_contacts(
            len(s_inds), {'s': contacts['s']})
        w_contacts, _ = Contacts.make_random_contacts(

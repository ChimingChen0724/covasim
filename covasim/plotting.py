...
__all__ = ['date_formatter', 'plot_sim', 'plot_scens', 'plot_result',
           'plot_compare', 'plot_people', 'plotly_sim', 'plotly_people', 'plotly_animate']
...
def handle_args(fig_args=None, plot_args=None, scatter_args=None, axis_args=None, fill_args=None,
    ...
    defaults.axis = sc.objdict(
        left=0.10, bottom=0.08, right=0.95, top=0.95, wspace=0.30, hspace=0.30)
    defaults.fill = sc.objdict(alpha=0.2)
    defaults.legend = sc.objdict(loc='best', frameon=False)
    defaults.date = sc.objdict(as_dates=True, dateformat=None,
                               interval=None, rotation=None, start_day=None, end_day=None)
    defaults.show = sc.objdict(
        data=True, ticks=True, interventions=True, legend=True)
    # Use Covasim global defaults
    defaults.mpl = sc.objdict(dpi=None, fontsize=None, fontfamily=None)
    ...
    if len(kwargs):
        ...
        valid = sc.strjoin(
            sorted(set([k for d in defaults.values() for k in d.keys()])))
...
def handle_to_plot(kind, to_plot, n_cols, sim, check_ready=True):
    ...
    if n_cols is None:
        ...
        # This gives 1 column for 1-4, 2 for 5-8, etc.
        n_cols = int((n_plots-1)//max_rows + 1)
    # Inconsistent naming due to Covasim/Matplotlib conventions
    n_rows, n_cols = sc.get_rows_cols(n_plots, ncols=n_cols)
...
def create_figs(args, sep_figs, fig=None, ax=None):
    ...
    if sep_figs:
        ...
        if fig is None:
            if ax is None:
                # Create the figure if none is supplied
...
# add new class "AddToPlot" to store methods "plot_data" and "plot_interventions"
...
class AddToPlot:
    def plot_data(sim, ax, key, scatter_args, color=None):
        ...
        if sim.data is not None and key in sim.data and len(sim.data[key]):
            ...
            ax.scatter(data_t, sim.data[key], c=[
                       color], label='Data', **scatter_args)
        return
...
# add new class "PlotStyle" to store methods "title_grid_legend"
...
# add new class "PlotChange" to initialize two classes,
# so that all methods in "PlotStyle" and "AddToPlot" can be directly called through "PlotChange"


class PlotChange:
    def __init__(self, subsystem1: AddToPlot, subsystem2: PlotStyle):
        self._subsystem1 = subsystem1 or AddToPlot()
        self._subsystem2 = subsystem2 or PlotStyle()

    def operate(self):
        self._subsystem1.plot_data()
        self._subsystem1.plot_interventions()
        self._subsystem2.title_grid_legend()
...
def reset_ticks(ax, sim=None, date_args=None, start_day=None):
    ...
    if date_args.start_day:
        # Keep original type (float)
        xmin = float(sc.day(date_args.start_day, start_day=start_day))
    ...
    if date_args.as_dates:
        ...
        date_formatter(start_day=start_day,
                       dateformat=date_args.dateformat, ax=ax)
...
def tidy_up(fig, figs, sep_figs, do_save, fig_path, do_show, args):
    ...
    if do_save:
        if fig_path is not None:  # No figpath provided - see whether do_save is a figpath
            # Ensure it's valid, including creating the folder
...
def plot_sim(to_plot=None, sim=None, do_save=None, fig_path=None, fig_args=None, plot_args=None,
    ...
    for pnum, title, keylabels in to_plot.enumitems():
        ax = create_subplots(figs, fig, ax, n_rows, n_cols,
                             pnum, args.fig, sep_figs, log_scale, title)
            ...
            if reskey in variant_keys:
                ...
                for variant in range(ns):
                    ...
                    if res.low is not None and res.high is not None:
                        # Create the uncertainty bound
                        ax.fill_between(
                            res_t, res.low[variant, :], res.high[variant, :], color=color, **args.fill)
                    # Actually plot the sim!
                    ax.plot(res_t, res.values[variant, :],
                            label=label, **args.plot, c=color)
            else:
                res = sim.results[reskey]
                color = set_line_options(
                    colors, reskey, resnum, res.color)  # Choose the color
                label = set_line_options(
                    labels, reskey, resnum, res.name)  # Choose the label
                if res.low is not None and res.high is not None:
                    # Create the uncertainty bound
                    ax.fill_between(res_t, res.low, res.high,
                                    color=color, **args.fill)
                ax.plot(res_t, res.values, label=label, **args.plot,
                        c=color)  # Actually plot the sim!
            if args.show['data']:
                AddToPlot.plot_data(
                    sim, ax, reskey, args.scatter, color=color)  # Plot the data
            if args.show['ticks']:
                # Optionally reset tick marks (useful for e.g. plotting weeks/months)
                reset_ticks(ax, sim, args.date)
        if args.show['interventions']:
            AddToPlot.plot_interventions(sim, ax)  # Plot the interventions
        if args.show['legend']:
            # Configure the title, grid, and legend
            PlotStyle.title_grid_legend(ax, title, grid, commaticks,
                                        setylim, args.legend)
...
def plot_scens(to_plot=None, scens=None, do_save=None, fig_path=None, fig_args=None, plot_args=None,
    ...
    args = handle_args(fig_args=fig_args, plot_args=plot_args, scatter_args=scatter_args, axis_args=axis_args, fill_args=fill_args,
                       legend_args=legend_args, show_args=show_args, date_args=date_args, mpl_args=mpl_args, **kwargs)
    to_plot, n_cols, n_rows = handle_to_plot(
        'scens', to_plot, n_cols, sim=scens.base_sim, check_ready=False)  # Since this sim isn't run
    fig, figs = create_figs(args, sep_figs, fig, ax)
    ...
    for pnum, title, reskeys in to_plot.enumitems():
        ...
        for reskey in reskeys:
            ...
            for snum, scenkey, scendata in resdata.enumitems():
                # Pull out the first sim in the list for this scenario
                sim = scens.sims[scenkey][0]
                variant_keys = sim.result_keys('variant')
                if reskey in variant_keys:
                    ns = sim['n_variants']
                    variant_colors = sc.gridcolors(ns)
                    for variant in range(ns):
                        ...
                        # Create the uncertainty bound
                        ax.fill_between(
                            scens.tvec, scendata.low[variant, :], scendata.high[variant, :], color=color, **args.fill)
                        ax.plot(scens.tvec, res_y, label=label, c=color,
                                **args.plot)  # Plot the actual line
                        if args.show['data']:
                            AddToPlot.plot_data(
                                sim, ax, reskey, args.scatter, color=color)  # Plot the data
                else:
                    res_y = scendata.best
                    color = set_line_options(
                        colors, scenkey, snum, default_colors[snum])  # Choose the color
                    label = set_line_options(
                        labels, scenkey, snum, scendata.name)  # Choose the label
                    # Create the uncertainty bound
                    ax.fill_between(scens.tvec, scendata.low,
                                    scendata.high, color=color, **args.fill)
                    ax.plot(scens.tvec, res_y, label=label, c=color,
                            **args.plot)  # Plot the actual line
                    if args.show['data']:
                        AddToPlot.plot_data(
                ...
                if args.show['interventions']:
                    AddToPlot.plot_interventions(
                        sim, ax)  # Plot the interventions
                if args.show['ticks']:
                    # Optionally reset tick marks (useful for e.g. plotting weeks/months)
                    reset_ticks(ax, sim, args.date)
        if args.show['legend']:
            # Configure the title, grid, and legend -- only show legend for first
            PlotStyle.title_grid_legend(ax, title, grid, commaticks,
                                        setylim, args.legend, pnum == 0)
...
def plot_result(key, sim=None, fig_args=None, plot_args=None, axis_args=None, scatter_args=None,
    ...
    # apply facade structural pattern
    # old: called 3 different methods: plot_data, plot_interventions, title_grid_legend
    # new: use the idea of structural pattern,
    #      create class "AddToPlot" to store method "plot data" and "plot_interventions"
    #      create class "PlotStyle" to store method "title_grid_legend"
    # create new class "PlotChange" to initialize two classes, so that all methods can be directly called through "PlotChange"
    # simplified the original way of calling into one line of code "facade.operate()"
    subsystem1 = AddToPlot()
    subsystem2 = PlotStyle()
    facade = PlotChange(subsystem1, subsystem2)
    facade.operate()
...
def plot_compare(df, log_scale=True, fig_args=None, axis_args=None, mpl_args=None, grid=False,
    ...
    axis_args = sc.mergedicts({'left': 0.16, 'bottom': 0.05, 'right': 0.98,
                              'top': 0.98, 'wspace': 0.50, 'hspace': 0.10}, axis_args)
    args = handle_args(fig_args=fig_args, axis_args=axis_args,
                       mpl_args=mpl_args, **kwargs)
...
def plot_people(people, bins=None, width=1.0, alpha=0.6, fig_args=None, axis_args=None,
    ...
    axis_args = sc.mergedicts(dict(
        left=0.05, right=0.95, bottom=0.05, top=0.95, wspace=0.3, hspace=0.35), axis_args)
    plot_args = sc.mergedicts(
        dict(lw=1.5, alpha=0.6, c=color, zorder=10), plot_args)

    # Compute statistics
    min_age = min(bins)
    max_age = max(bins)
    # Add an extra bin to end to turn them into edges
    ...
    pl.bar(bins, age_counts, color=color,
           alpha=alpha, width=width, zorder=zorder)
    ...
    for lk in lkeys:
        ...
        contact_counts[lk] = np.histogram(
            p1ages, edges)[0] + np.histogram(p2ages, edges)[0]
    ...
    for w, w_type in enumerate(['total', 'percapita', 'weighted']):
        for i, lk in enumerate(lkeys):
            ...
            if w_type == 'total':
                ...
                # Proportion of people that have contacts in this layer
                participation = n_members/len(people)
                title = f'Total contacts for layer "{lk}": {total_contacts:n}\n({participation*100:.0f}% participation)'
            elif w_type == 'percapita':
                age_counts_within_layer = np.histogram(
                    people.age[members_lk], edges)[0]
                weight = np.divide(1.0, age_counts_within_layer,
                                   where=age_counts_within_layer > 0)
                # Factor of 2 since edges are bi-directional
                mean_contacts_within_layer = 2*n_contacts/n_members if n_members else 0
            ...
            ax = pl.subplot(n_rows, n_layers, n_layers *
                            (w+1)+i+1, sharey=share_ax)
            pl.bar(bins, contact_counts[lk]*weight, color=layer_colors[i],
                   width=width, zorder=zorder, alpha=alpha)
            pl.xlim([min_age-offset, max_age+offset])
            pl.xticks(np.arange(0, max_age+1, gridspace))
            pl.grid(True)
            pl.xlabel('Age')
            ...
            if hasattr(interv, 'days'):
                for interv_day in interv.days:
                    if interv_day and interv_day < sim['n_days']:
                        ...
                        fig.add_shape(dict(type="line", xref="x", yref="paper", x0=interv_date,
                                      x1=interv_date, y0=0, y1=1, line=dict(width=0.5, dash='dash')))
                        if add_to_legend:
                            fig.add_trace(go.Scatter(x=[interv_date], y=[
                                          0], mode='lines', name='Intervention change', line=dict(width=0.5, dash='dash')))
    return
    ...
    for p, title, keylabels in to_plot.enumitems():
        ...
        for key in keylabels:
            ...
            fig.add_trace(go.Scatter(x=x, y=y, mode='lines',
                          name=label, line_color=this_color))
            if sim.data is not None and key in sim.data:
                xdata = sim.data['date']
                ydata = sim.data[key]
                fig.add_trace(go.Scatter(x=xdata, y=ydata, mode='markers',
                              name=label + ' (data)', line_color=this_color))

        # Only add the intervention label to the legend for the first plot
        plotly_interventions(sim, fig, add_to_legend=(p == 0))
        fig.update_layout(
            title={'text': title}, yaxis_title='Count', autosize=True, **plotly_legend)
...
def plotly_people(sim, do_show=False):
    ...
    fig.update_layout(title={'text': 'Numbers of people by health state'},
                      yaxis_title='People', autosize=True, **plotly_legend)
...
def plotly_animate(sim, do_show=False):
    ...
    z = np.pad(z, ((0, x_size * y_size -
               z.shape[0]), (0, 0)), mode='constant', constant_values=np.nan)
